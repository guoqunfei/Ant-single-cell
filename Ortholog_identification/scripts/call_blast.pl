...
while (my $file = readdir IN) {
	...
	open SH, ">$sh_file";
	print SH "date\n$program -p $blast_type -i $query_file -d $database_file -o $out_file -F F -a 2 -e 1e-5 -m 8\ndate\n";
	close SH;
	system "qsub -S /bin/sh -cwd -l vf=${mem}G,num_proc=1 -q $qsub_q -P $qsub_p -binding linear:1 $sh_file";
}
closedir IN;
